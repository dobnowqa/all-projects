...
/*				check(Constants.el_sow_service_work);
				check(Constants.el_sow_general_wiring);
				check(Constants.el_sow_lighting_work);
				select(Constants.el_sow_lighting_work_type, data[3]);*/
				int completed = 0;
				if(!data[1].contains("null"))
					check("//input[@name='togglechk1']");
				if(!data[2].contains("null"))
					check("//input[@name='togglechk2']");
				if(!data[3].contains("null"))
					check("//input[@name='togglechk3']");
				if(!data[3].contains("null"))
					select("//select[@ng-model='FormED16AObj.CategoryofWork.LightingWorktype']", data[3]);
				if(!data[4].contains("null"))
					check("//input[@name='togglechk4']");
				if(!data[5].contains("null"))
					check("//input[@name='togglechk5']");
				if(!data[6].contains("null"))
					check("//input[@name='togglechk6']");
				if(!data[7].contains("null"))
					check("//input[@name='togglechk7']");
                // OTHER
				if(!data[8].contains("null")) {
					select("//select[@id='selCategoryWorklist']", data[3]);
					if(!data[8].contains("Elevators/Escalator/Material Lift"))
						type("//input[@id='txtElevatorJobAppNum']", "12345");
					if(!data[8].contains("Power Generation"))
						check("//input[@id='powergenCogen']");
					
					
				}
					
				if (count(Constants.completed_checkmark) >= completed) {
